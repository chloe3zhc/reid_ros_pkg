...
import time
from cluster_manager import ClusteredMemoryManager, assign_id_to_feature
...
class reid_node:
    ...
    def load_models(self):
        """加载YOLO和ReID模型"""
        # Load YOLO model
        self.yolo_model = YOLO("./yolo11n.pt")  # Load an official Detect model
        ...
        self.reid_model.load_param_ignore_classifier("./FusionReID_180.pth")  # Load trained weights
    ...
    def image_callback(self, data):
        ...
        try:
            ...
            # 发布结果信息
            result_msg = String()
            result_msg.data = f"Processed image with {len([r for r in getattr(self, 'reid_results', []) if r['reid_id'] is not None])} re-identified objects"

...
class reid_node_0:
    def __init__(self):
        ...
        # 获取目标图像路径参数
        self.target_image_path = rospy.get_param('~target_image_path', '/home/u/Pictures/zhc_3.jpg')

        # 相似度阈值
        self.similarity_value = rospy.get_param('~similarity_value', 0.85)
    ...
    def process_target_with_color_coding(self, frame):
        ...
        # 找到相似度最高的目标（如果存在超过阈值的目标）
        best_result = None
        for result in all_results:
            if result['similarity'] >= self.similarity_value:  # 阈值可以根据需要调整
                if best_result is None or result['similarity'] > best_result['similarity']:
                    best_result = result

        # 在帧上绘制目标，只绘制相似度最高的目标（如果存在超过阈值的目标）
        annotated_frame = frame.copy()
        # 如果存在超过阈值的目标，只绘制相似度最高的那个
        if best_result is not None:
            x1, y1, x2, y2 = best_result['bbox']
            reid_id = best_result['reid_id']
        ...
        # # 可选：绘制低于阈值的目标（用不同颜色）
        # for result in all_results:
        #     if result['similarity'] < self.similarity_value:
        #         x1, y1, x2, y2 = result['bbox']
        #         reid_id = result['reid_id']
        #         similarity = result['similarity']
                
        #         color = (0, 0, 255)  # 红色表示匹配度低

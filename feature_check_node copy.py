...
class feature_check_node:
    def __init__(self):
        ...
        self.save_dir = rospy.get_param('~save_dir', '/home/u/pictures/reid_images_2')  # 保存目录
        os.makedirs(self.save_dir, exist_ok=True)  # 创建保存目录
        rospy.loginfo("dir initialized")
    ...
    def process_reid(self, frame):
        ...
        '''results.boxes(xyxy, conf, cls, id);results.orig_img;results.names
        xyxy: tensor([[100, 150, 200, 300],   # 边界框坐标 [x1, y1, x2, y2]
                        [300, 120, 400, 280],
                        [500, 100, 600, 250]])
        conf: tensor([0.95, 0.92, 0.88]),     # 置信度
        cls: tensor([0, 0, 0]),               # 类别
        '''

...
class reid_node_0:
    ...
    def process_reid(self, frame):
        ...
        if results[0].boxes is not None:
            for i, box in enumerate(results[0].boxes):
                ...
                # 使用 assign_id_to_feature 分配ID
                assigned_id = self.assign_id_to_feature(feature, self.known_features, self.known_ids, threshold=0.9)
                
                if assigned_id is None:
                    # 如果没有匹配的ID，则创建新ID
                    new_id = max(self.known_ids, default=-1) + 1
                    assigned_id = new_id
                    self.known_features.append(feature)
                    self.known_ids.append(assigned_id)
    ...
    def assign_id_to_feature(self, feature, known_features, known_ids, threshold=0.9):
        """
